...
class VentasVivoService {
  final String ventasURL = '$apiUrl/ventas_vivos';

  Future<List<VentaVivo>> getVentas(String token, DateTime ini, DateTime fin) async {
    try {
      ...
      if (map['ok']) {
        return List<VentaVivo>.from(map['rows'].map((e) => VentaVivo.fromJson(e)));
      } else {
  ...
  Future<List<VentaVivo>> getVenta(int id, String token) async {
    try {
      final response = await http.get(
      ...
      final map = jsonDecode(response.body);
      if (map['ok']) {
        final venta = VentaVivo.fromJson(map['venta']);
        return [venta];
      } else {
        throw Exception(map['message']);
      }
  ...
  Future<bool> insertVenta(VentaVivo venta, String token) async {
    try {
  ...
  Future<bool> editarPrecio(VentaVivo venta, String token) async {
    try {
  ...
  Future<bool> descontar(VentaVivo venta, String token) async {
    try {
      final response = await http.patch(
  ...
  Future<bool> anular(VentaVivo venta, String token) async {
    try {
      final response = await http.patch(
