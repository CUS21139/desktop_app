...
import '../../../services/productos_vivo_service.dart';
...
class ProductosVivoView extends StatefulWidget {
  const ProductosVivoView({super.key});

  @override
  State<ProductosVivoView> createState() => _ProductosVivoViewState();
}

class _ProductosVivoViewState extends State<ProductosVivoView> {
  final productoCtrl = TextEditingController();

  List<ProductoVivo> filtro = [];

  bool isSearching = false;

  void filter() {
    final service = Provider.of<ProductosVivoProv>(context, listen: false);
    isSearching = productoCtrl.text.isNotEmpty;
    List<ProductoVivo> pesadores = [];
  ...
  Widget build(BuildContext context) {
    return Flex(
      ...
      children: [
        ...
        Expanded(
          child: Consumer<ProductosVivoProv>(builder: (_, service, __) {
  ...
  void refresh() async {
    final service = ProductosVivoService();
    final productosProv = Provider.of<ProductosVivoProv>(context, listen: false);
  ...
  void update(BuildContext context, ProductoVivo producto) {
    final service = ProductosVivoService();
    final productosProv = Provider.of<ProductosVivoProv>(context, listen: false);
    final token = Provider.of<UsuariosProv>(context, listen: false).token;
  ...
  void delete(BuildContext context, ProductoVivo producto) {
    final service = ProductosVivoService();
    final productosProv = Provider.of<ProductosVivoProv>(context, listen: false);
    final token = Provider.of<UsuariosProv>(context, listen: false).token;
    showDialog(
